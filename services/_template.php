...
<body class="case-page page-service">
  ...
  <main class="svc">
    ...
    <!-- Отзывы: та же лента, что на главной, и всегда на тёмной плите —
         ...
         Не зависит от наличия кейсов — у услуги может не быть своей
         категории портфолио (например, продвижение), а отзывы всё
         равно к месту. -->
    <?php $forcePlate(true); ?>
    ...
    <?php /* Калькулятор — своё условие, отдельное от кейсов: у услуги может
             не быть подходящей категории портфолио (см. выше), но цену
             всё равно нужно показывать. Выключается чекбоксом в админке
             («Показывать калькулятор на странице») — калькулятор скрыт,
             если его явно выключили; по умолчанию (поле не задано) — виден. */
    if (($calc['enabled'] ?? true) !== false): ?>
